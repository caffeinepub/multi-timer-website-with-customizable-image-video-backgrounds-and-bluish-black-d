...
const CustomSoundsContext = createContext<CustomSoundsContextType | undefined>(
  undefined,
);
...
export function CustomSoundsProvider({
  ...
  const addSoundFromFile = useCallback(
    async (name: string, file: File): Promise<boolean> => {
      ...
      if (!isValidAudioFile(file)) {
        toast.error(
          "Invalid audio file. Please use MP3, WAV, OGG, or similar audio formats.",
        );
        return false;
      }
      ...
      if (file.size > 1024 * 1024) {
        toast.error(
          "File is too large. Please use files smaller than 1MB or provide a direct URL instead.",
        );
        return false;
      }
      ...
      try {
        ...
        if (!testResult.success) {
          toast.error(
            testResult.error || "Failed to save sound. File may be too large.",
          );
          return false;
        }
    ...
    async (url: string) => {
      try {
        ...
        if (isYouTubeUrl(url)) {
          ...
          if (!success) {
            toast.error(
              "Failed to play YouTube sound. The URL may be invalid or the video may be unavailable.",
            );
          }
          return;
        }
        ...
        toast.error(
          "Failed to play sound. The URL may be invalid or unreachable.",
        );
      }
    },
    [volume],
