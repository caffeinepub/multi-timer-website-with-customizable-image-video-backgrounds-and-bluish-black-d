...
export function saveCustomSounds(sounds: CustomSound[]): {
  success: boolean;
  error?: string;
} {
  try {
    ...
    if (sizeInBytes > MAX_STORAGE_SIZE) {
      return {
        success: false,
        error:
          "Storage limit exceeded. Try using shorter URLs or removing some sounds.",
      };
    }
    ...
    if (!success) {
      return {
        success: false,
        error:
          "Storage quota exceeded. Try removing some sounds or using shorter URLs.",
      };
    }
