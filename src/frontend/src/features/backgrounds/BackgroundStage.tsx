...
const YouTubeBackgroundPlayerWithControls = memo(
  function YouTubeBackgroundPlayerWithControls({
    videoId,
    volume,
    isPlaying,
    onError,
...
export const BackgroundStage = memo(function BackgroundStage() {
  ...
  useEffect(() => {
    if (
      videoRef.current &&
      mediaType === "video" &&
      backgroundUrl !== prevVideoUrlRef.current
    ) {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      videoRef.current.play().catch(() => {
        setVideoError(
          "Video playback failed. Your browser may not support this format.",
        );
      });
    } else {
  ...
  const handleImageError = useCallback(() => {
    const errorMsg =
      "Image failed to load. The URL may be unreachable or the format may not be supported.";
  ...
  const handleVideoError = useCallback(() => {
    const errorMsg =
      "Video failed to load. The URL may be unreachable or the format may not be supported.";
  ...
  const handleYouTubeError = useCallback(
    (errorMsg: string | null) => {
      ...
      if (
        ...
        !errorMsg.toLowerCase().includes("sound")
      ) {
        clearBackground();
      }
    },
