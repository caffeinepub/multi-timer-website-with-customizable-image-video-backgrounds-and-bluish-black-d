...
const TimerAlertsContext = createContext<TimerAlertsContextType | undefined>(
  undefined,
);
...
export function TimerAlertsProvider({
  children,
  ...
  const getCustomSoundUrl = useCallback(
  ...
  const previewSound = useCallback(
    async (sound: AlertSoundOption) => {
      const customUrl = sound.startsWith("custom-")
        ? getCustomSoundUrl(sound)
        : undefined;
      const success = await playAlertSound(sound, customUrl);

      if (!success && sound.startsWith("custom-")) {
        toast.error(
          "Could not play custom sound. The URL may be invalid or the video may be unavailable.",
        );
      }
    },
    [getCustomSoundUrl],
  ...
  const notifyCompletion = useCallback(
    async (message: string) => {
      if (alertsEnabled) {
        ...
        const customUrl = completionSound.startsWith("custom-")
          ? getCustomSoundUrl(completionSound)
          : undefined;
        const success = await playAlertSound(completionSound, customUrl);

        if (!success && completionSound.startsWith("custom-")) {
          toast.error(
            "Could not play completion sound. The URL may be invalid or playback may be blocked by your browser.",
          );
        }
      }
    },
